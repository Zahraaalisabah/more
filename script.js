...
// ==========================================
// متغيرات عامة لتقاسم الحالة بين الوظائف
// ==========================================
...
const SUPABASE_KEY = "sb_publishable_E71VdNUu5WqVdLlBt6Z8kg_snM-MUdj";
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    try {
        cart = JSON.parse(localStorage.getItem("more_cart")) || [];
    } catch (error) {
    ...
    function updateCartUI() {
        const totalItemsCount = cart.reduce(function (sum, item) {
        ...
        if (cartHeaderCount) {
            cartHeaderCount.textContent = `${totalItemsCount} منتجات`;
        }
        ...
        cart.forEach(function (item, index) {
            ...
            const itemTotal = priceNumeric * quantity;
    ...
    if (loginAdminBtn) {
        loginAdminBtn.addEventListener("click", function () {
            ...
            if (adminPasswordInput.value === ADMIN_PASSWORD) {
                isAdminLoggedIn = true; // تحديث حالة المشرف
    ...
    if (logoutAdminBtn) {
        logoutAdminBtn.addEventListener("click", function () {
            isAdminLoggedIn = false; // إلغاء صلاحية المشرف عند تسجيل الخروج
...
// ==========================================
// دالة حذف المنتج من Supabase (مع التحقق من صلاحية الأدمن)
// ==========================================
async function deleteProduct(productId) {
    // فحص ما إذا كان المستخدم أدمن
    if (!isAdminLoggedIn) {
        alert("لا يمكنك حذف المنتج! هذه الصلاحية للمدير فقط.");
        return;
    }

    // إذا كان أدمن يُطلب منه التأكيد
    ...
    try {
        ...
        alert("لا يمكنك حذف المنتج أو حدث خطأ أثناء الحذف.");
    }
}
